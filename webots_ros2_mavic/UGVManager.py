...
class UGVManager(Node):
    def __init__(self, ugvName, assignedPOIs):
        self.name = ugvName
        self.assignedPOIs = assignedPOIs
        super().__init__(self.name)
        self.publisher = self.create_publisher(Twist, "/" + self.name + "/cmd_vel", 10)
        self.subscriber = self.create_subscription(Float64MultiArray, "/" + self.name + "/TurtleBot3Burger_" + self.name[len(self.name) - 1] + "/gps", self.subCallback, 10)
...
def main(argv):
    ...
    ugvManager = UGVManager(argv[2], argv[4])
    rclpy.spin(ugvManager)
    rclpy.shutdown()
