...
K_X_VELOCITY_P = 0.00001          #Original: 1
K_Y_VELOCITY_P = 0.00001          #Original: 1

K_X_VELOCITY_I = 0.0001       #Original: 0.01
K_Y_VELOCITY_I = 0.0001       #Original: 0.01
...
class MavicAutonomy:
    def init(self, webots_node, properties):
        ...
        self.__target_twist = Twist()
        self.__target_twist.linear.x = 10.0
        self.__target_twist.linear.y = 0.0
        self.__target_twist.linear.z = 1.0
        self.__vertical_ref = LIFT_HEIGHT
        self.__linear_x_integral = 0
        self.__linear_y_integral = 0
    ...
    def __move_to_target(self):
        ...
        #if '0' in self.__robot.getName():
            #self.__node.get_logger().info(f"pitch: {pitch_disturbance}\n")
    ...
    def step(self):
        ...
        if elapsedTime < 5 * int(self.__mavicNumber):
            ...
            #ORIGINAL: roll_input = K_ROLL_P * clamp(roll, -1, 1) + roll_velocity + roll_ref
            #ORIGINAL: pitch_input = K_PITCH_P * clamp(pitch, -1, 1) + pitch_velocity + pitch_ref
