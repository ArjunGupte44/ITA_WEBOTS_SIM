...
class Config(object):
    ...
    reward = BaseConfig()
    # reward 应改为字典格式好一些
    reward.success_reward_low = 10
    reward.success_reward_medium = 20
    reward.success_reward_high = 30
    reward.penalty_reward_low = -10
    reward.penalty_reward_medium = -20
    reward.penalty_reward_high = -30
    reward.gamma = 0.99
    ...
    # UAV config
    UAV = BaseConfig()
    # whether UAV is visible to humans (whether humans respond to the UAV's motion)
    #UAV.visible = False
    # number of UAV
    #UAV.number = 4
    # For baseline: srnn; our method: selfAttn_merge_srnn
    #UAV.policy = 'selfAttn_merge_srnn'
    #UAV.radius = 0.3
    UAV.v_pref = 20
    ...
    # radius of perception range
    UAV.sensor_range = 5
    # image quality of UAV, 0 is low
    UAV.Image_quality = 0

    # UGV config
    UGV = BaseConfig()
    # whether UGV is visible to humans (whether humans respond to the UGV's motion)
    #UGV.visible = False
    # For baseline: srnn; our method: selfAttn_merge_srnn
    #UGV.policy = 'selfAttn_merge_srnn'
    #UGV.radius = 0.3
    UGV.v_pref = 8
    #UGV.sensor = "coordinates"
    # FOV = this values * PI
    #UGV.FOV = 2
    # radius of perception range
    UGV.sensor_range = 5
    # image quality of UGV, 1 is high
    UGV.Image_quality = 1
    
    # threat config,是不是放到sim里好一些，之后斟酌一下
    POI = BaseConfig()
    POI.number = 50
    #POI.policy = 'random'
