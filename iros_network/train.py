	...
	for j in range(num_updates):
		...
		for step in range(algo_args.num_steps):
			...
			with torch.no_grad():
				...
				value, action, action_log_prob = actor_critic.act(rollouts_obs)
				#print(action,action.shape)
				#action 返回一个坐标，即robot的gx,gy。应改为机器人组的gx,gy
			# if we use real prediction, send predictions to env for rendering

			'''if env_name == 'CrowdSim-v0' and env_config.env.use_wrapper:
		...
		if j % algo_args.log_interval == 0 and len(episode_rewards) > 1:
			...
			df = pd.DataFrame({'misc/nupdates': [j], 'misc/total_timesteps': [total_num_steps],
							   ...
							   'loss/value_loss': value_loss,'mean reward':np.mean(episode_rewards),
							'median reward':np.median(episode_rewards),'min_reward':np.min(episode_rewards),
